...
//
// Reads hyperfine's JSON exports (`benches/results/{noop,hello}.json`, produced
// by `benches/run.sh`) and writes a slim, committed snapshot the benchmarks page
...
const data = {
  meta: {
    ...
    harness: "hyperfine, --shell=none, 20 warmup runs",
  },
  scenarios: {
    noop: { label: "no-op — spawn `true`", results: load("noop") },
    hello: { label: "`echo hello`", results: load("hello") },
  },
};

mkdirSync(dirname(outFile), { recursive: true });
writeFileSync(outFile, JSON.stringify(data, null, 2) + "\n");
console.log(`wrote ${outFile} (${data.scenarios.noop.results.length} runners)`);
